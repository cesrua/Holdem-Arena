...
$(document).ready(function () {
    ...
    $("#slider").slider({
        range: "min", value: 0, min: 0, max: 100, step: 0.5, slide: function (a, b) {
...
function onPercentChanged() {
    ...
    a = a.replace(/[^0-9.]/g, "");
    document.getElementById("slider-value").value = a; $("#slider").slider({
        value: a
    });
    changeGrid();
    showRangeLink()
...
//función que enseña el rango de cartas seleccionado.
...
// Función para generar el link del grid

function showRangeLink() {
    var a = "http://" + location.host + "/range/" + document.getElementById("slider-value").value;
    if (vs != 3) { a = a + "?vs=" + vs } document.getElementById("link_range").value = a
...
function vsChanged(a) {
    vs = getCheckedValue(a); setHandListForVs();
    changeGrid();
    showRangeLink()
}
