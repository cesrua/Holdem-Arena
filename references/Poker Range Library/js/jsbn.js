...
function am1(f, a, b, e, h, g) {
    while (--g >= 0) {
        ...
        h = Math.floor(d / 67108864); b[e++] = d & 67108863
    }
    return h
}

function am2(f, q, r, e, o, a) {
    var k = q & 32767, p = q >> 15; while (--a >= 0) {
        var d = this[f] & 32767;
        var g = this[f++] >> 15;
        var b = p * d + g * k;
...
function am3(f, q, r, e, o, a) {
    ...
    while (--a >= 0) {
        var d = this[f] & 16383; var g = this[f++] >> 14;
        var b = p * d + g * k;
...
function bnpDivRemTo(n, h, g) {
    ...
    f.subTo(d, d); while (d.t < p) {
        d[d.t++] = 0
    }
    while (--s >= 0) {
        var c = (g[--u] == b) ? this.DM : Math.floor(g[u] * A + (g[u - 1] + x) * z);
        if ((g[u] += d.am(0, c, g, s, 0, p)) < c) {
            d.dlShiftTo(s, f); g.subTo(f, g);
            while (g[u] < --c) {
                g.subTo(f, g)
            }
        }
...
function montConvert(a) {
    var b = nbi(); a.abs().dlShiftTo(this.m.t, b);
...
function montReduce(a) {
    ...
    for (var c = 0; c < this.m.t; ++c) {
        ...
        b = c + this.m.t; a[b] += this.m.am(0, d, a, c, 0, this.m.t);
        while (a[b] >= a.DV) {
            a[b] -= a.DV; a[++b]++
        } 
    }
...
function bnpExp(h, j) {
    ...
    d.copyTo(f); while (--c >= 0) {
        j.sqrTo(f, a);
        if ((h & (1 << c)) > 0) {
            j.mulTo(a, d, f)
        }
        else { var b = f; f = a; a = b } 
    }
    return j.revert(f)
}
